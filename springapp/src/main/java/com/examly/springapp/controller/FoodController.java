...
public class FoodController {
    ...
    public List<Food> getId(@PathVariable int foodId){
        return api.getFoodId(foodId);
    }

    @GetMapping ("/origin/{foodOrigin}" )
    public List<Food> getpop(@PathVariable String foodOrigin) {
        return api.getFoodOrgin(foodOrigin);
    }

    @GetMapping("/name/{foodName}")
    public List<Food> getname(@PathVariable String foodName){
